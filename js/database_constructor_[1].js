...
function LNAU_DataBase (dataBaseId) {
		...
		this.test_response = function ($response) {
			...
			if (lnau.crossDatabaseProcessor.requestTo[this.id].status) {
				if (this.isEmptyValue($response.result)) { return false; }
		...
		this.sendRequestToAppendNewRecord = function ($record) {
			this.worker.postMessage( { request_type:4, record:$record } );
		};
		...
		this.showDataAsTable = function ($caption, $fields, $labels) {
			...
			for (var rowNum = 0; rowNum < this.data.length; rowNum++) {
				...
				for (var cellNum = 0; cellNum < $fields.length; cellNum++) {
					...
					cell.innerHTML = this.getFieldValue (rowNum, $fields[cellNum]); 
				}
			}
			document.getElementById("main_content").innerHTML = '';
		...
		this.getReferenseFieldValue = function (recordNum, fieldName) {
			...
			return lib[$code]?lib[$code].data:'<span style="color:red">Значення не визначене</span>';
		};
		...
		this.getFieldValue = function (recordNum, $field) {
			var _index = this.getFieldIndex($field);
			if ( this.common_data.fields.ref && this.common_data.fields.ref[_index] ) {
				return this.getReferenseFieldValue (recordNum, $field);
			}
		...
		this.getInputField = function (objType, recNum, $field) {
			...
			// console.info('type = ' + objType +  '   ' + index + ' : ' + $field);
			...
			if ( fieldRef ) {
				var inputField = document.createElement('span');
				//inputField.innerHTML = this.getReferenseFieldValue (recNum, $field);
				//_container.appendChild(inputField);
				//inputField.style.color = 'black';
				inputField = this.getReferenseFieldInput (recNum, $field);
				_container.appendChild(inputField);
				return _container;
			}
