...
this.saveData = function () {
	if (Array.isArray(worker_phpURL) && lnau.common_data.saveByKey) {
		...
		for (var j=0; j < worker_phpURL.length; j++) {
			...
			tmp = j === 0 ? { common_data:lnau.common_data, info:tmp } : { info:tmp }
	...
	switch (responseHeader.responseType) {
		...
		case  1:
				...
				setTimeout( function () {
					    if ( eval (_test) ) {
							...
							postMessage( {
								...
								status:false,
								error_message:'Час очікування витік'
							} ) 
						}
				}, 4000)
			}
		    ...
		    if (responseHeader.keyFields && responseHeader.keyValues) {
			    ...
			    responseHeader.keyFields = Array.isArray( responseHeader.keyFields ) ?
							responseHeader.keyFields : [responseHeader.keyFields]
			    responseHeader.keyValues = Array.isArray(responseHeader.keyValues) ?
				    			responseHeader.keyValues : [responseHeader.keyValues]
		...
		case  2:
			...
			if (err1 || err2 || err3) {
				var $error_message = err1 ?
				    	'Не вказаний номер запису' :
					( err3 ? 'Немає даних'  : 'Запису з таким номером не існує' )
				postMessage( {
					header:responseHeader, 
					status:false,
					error_message:'Заміна запису: ' + $error_message
				})
			}
		...
		case  3:
			...
			if ( !e.data.params || !e.data.params.data ) {
				postMessage( {
					header:responseHeader,
					status:false,
					error_message:'Заміна даних : Немає даних'
				} )
			}
			else {
		...
		case  4:
		    ...
		    if (!e.data.params.record) {
				postMessage( {
					header:responseHeader,
					status:false,
					error_message:'Додавання запису : Немає даних'
				} )
			}
			else {
		...
		case  5:    
			...
			if (!Number.isInteger(responseHeader.recordNum) || !lnau.data[responseHeader.recordNum]) {
				var $error_message = !Number.isInteger(responseHeader.recordNum) ? 
				    	'Не визнечено номер запису' : 'Запису з таким номером не існує'
				postMessage ({
					header:responseHeader,
					status:false,
					error_message:'Видалення запису: ' + $error_message
				})
			}
			else {
